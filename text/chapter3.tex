...
Fine-grained parallelization is built into the \texttt{pegSerial} program, and enabled using the \texttt{--threads} command-line argument.
TODO: scalability

The coarse-grained parallelization lends itself naturally to distributed-memory computing, and so we implemented it using the Message Passing Interface (MPI) standard \cite{mpi08}.  In this environment, the same program (\texttt{pegMPI}) is launched simultaneously on multiple nodes in cluster.  Each node determines for itself which wavelengths/energies/incidence angles it should calculate.  After every calculation, each node broadcasts its results over the network to the ``root node'', which is responsible for writing a single output file and progress file.
