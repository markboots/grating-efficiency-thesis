...
Table \ref{tableMPITiming} shows that the coarse-grained parallelization is extremely efficient; in fact, it seems to run more efficiently than the single-threaded program.  This is of course impossible; the efficiencies over 100\% are simply timing artifacts caused by the fact that some \texttt{bugaboo} nodes are faster than others.  
...
We tested the new software extensively for logical self-consistency, and for consistency with \texttt{Gradif}.  In this section, we list the tests that give us confidence that its output is at least physically reasonable; in the next chapter, we validate both \texttt{Gradif} and our new software against experimental measurements.
...
\end{itemize}
\end{itemize}
For real gratings with absorbing materials, such as Platinum at 410 eV:
\begin{itemize}
