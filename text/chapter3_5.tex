...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[scale=0.8]{Extended/OptimalIncidenceGlobalRect/results.pdf} 
   \caption[Optimizing the incidence angle and groove geometry for a range of wavelengths and grating periods shows a $n=-1$ order maximum at the $n=+1$ order Wood anomaly: $\sin \theta_{max} = 1 - \lambda/d$.]{Optimizing the incidence angle and groove geometry for a range of wavelengths and grating periods shows a $n=-1$ order maximum efficiency at angles described by $\sin \theta_{max} = 1 - \lambda/d$ [smooth curves].  This is the angle where the $n=+1$ order reaches 90\dg: a Wood anomaly.  The optimization was conducted on a grid with an incidence step of 0.1\dg and a groove depth step of 0.002~um, using rectangular platinum gratings.}
...
TODO: waiting for these calculations to finish.
