...
Figures \ref{3i-2} and TODO contains a take-away message for instrument designers: if an arbitrary incidence angle is required for some external reason, the geometry (blaze angle, depth, etc.) can be adjusted to optimize for that incidence. However, in the absence of constraints, there exists a unique optimal incidence angle (and corresponding groove geometry) where the efficiency reaches an absolute maximum.

TODO: highlight example: max eff using 88 deg.  Achievable at max incidence?  [use 100 eV]
