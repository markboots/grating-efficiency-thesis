...
The purpose of the reflectometer endstation (Figure \ref{5b}) is to measure the intensity of light reflected off a sample -- in our case, a grating -- as a function of both incidence and reflection angle.  We can use it to determine grating efficiency by positioning the grating at the intended incidence angle relative to the incoming beam,  measuring the intensity at the angle of the outgoing order, and comparing this to the initial beam intensity.  Upstream, the monochromator and order suppressor are used to produce the monochromatic incident beam and set its energy for each measurement point.

In the reflectometer mechanics, a ``two-circle goniometer'' provides independent control over the angle of the sample and the angle of the detector arm, as well as providing precise (4 um) positioning to align the sample in $x$, $y$, and $z$.  The sample holder can accommodate samples up to 200 mm in diameter, which allowed us to mount two of our gratings side-by-side at once.  The detector arm contains a photodiode, a channel electron multiplier (CEM), and a CCD camera; for our grating measurements, we used the photodiode covered by a 2 mm pinhole.

\subsubsection{Diffraction Experiment Procedure}

\begin{enumerate}
\item Grating Alignment
	\begin{enumerate}
	\item asdf
	\end{enumerate}
	
\item Scanning (and scan modes)
