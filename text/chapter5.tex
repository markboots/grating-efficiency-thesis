...
Given the randomness of the surface structure, we need statistical methods to answer these questions.  Statistically, the surface roughness is characterized by $\sigma$, the root mean square (RMS) variation in height from the nominal plane, and by $P(z)$, the probability distribution function for the height.

Even for the simpler (non-grating) case of roughness on flat surfaces, a substantial amount of research has only succeeded in producing a sea of approximate models. (TODO REF Elfouhaily and Guerin) categorized 260 references -- 177 since 1980 -- into 30 different methods, before concluding that "there does not seem to be a universal method that is to be preferred systematically. All methods present a compromise between versatility, simplicity, numerical efficiency, accuracy and robustness, with a different weighting in these various fields. [...] No approximate model has fulfilled all listed criteria."

Even though they only deal with nominally flat surfaces, all of these methods are based on the assumption that a random rough surface can be decomposed into a superposition of sinusoidal gratings\footnote{Obviously, of very high groove density!}, and that scattering can be explained as a diffraction effect from the phase changes created by the ``microtopography'' of the surface.  (TODO REF Modified Beckmann-Kirchhoff scattering model for rough surfaces with large incident and scattering angles
Opt. Eng. 46, 078002 (Jul 02, 2007); doi:10.1117/1.2752180)  There are two fundamental approaches: the Rayleigh-Rice theory (TODO REF), which is a vector perturbation method for periodic surfaces, or the Beckmann-Kirchoff theory, which uses a scalar wave equation and applies the Kirchoff diffraction formula (TODO REF P. Beckmann and A. Spizzichino, The Scattering of Electromagnetic Waves from Rough Surfaces; Pergamon, New York, 1963, Chaps. 4 and 5.).  Both contain approximations; the Rayleigh-Rice methods are only applicable to what are known as ``smooth'' surfaces ($\sigma << \lambda$), and the Beckmann-Kirchoff methods contain a small-angle assumption that restricts them to near-normal incidence and scattering.

Since approximated grating theories are already being used to describe diffuse scattering from rough \emph{flat} surfaces, applying these methods to \emph{actual} gratings is even more difficult; this amounts to creating a superposition of (very high density) gratings, \emph{on the surface of a grating}(!) TODO REF (Angular distribution of light scattered from a sinusoidal grating 1 September 2000 Vol. 39, No. 25 APPLIED OPTICS) looks at this scenario for rough sinusoidal gratings illuminated by coherent visible light, and confirms that roughness increases the background light level. (TODO wish we could say more...)  TODO L. Nevot, P. Croce, Rev. Phys. Appl. 15, 761 (1980). 10.1051/rphysap:01980001503076100  Says What?

For gratings, (TODO REF http://www.gratinglab.com/information/handbook/chapter10.asp\#10.1.1) reports that within the plane, the intensity of diffuse scattering is stronger at angles near the diffraction orders than it is between orders.  (TODO REF M. R. Sharpe and D. Irish, "Stray light in diffraction grating monochromators," Opt. Acta 25, 861-893 (1978)) analyzes scattering specifically from gratings, and indicates that the intensity of scattering increases as the fourth power of the energy.
 
 Because of the difficulty in analyzing gratings structured on top of gratings, we apply some of the simplified results for rough mirrors to come up with a reasonable correction for our grating efficiencies.  The X-ray Data Booklet (TODO REF) gives a correction factor for the reflectivity $R'$ of a real mirror, where $R$ would be the reflectivity of an ideally smooth surface:
\begin{eqnarray}
R' &=& R \exp\left(  - \left(\frac{4\pi\sigma}{\lambda}\right)^2 \mathbf{Re}\left[ \sin \theta \sqrt{n^2 - \sin^2 \theta}  \right]    \right)
\end{eqnarray}
where $\sigma$ is the RMS value of the surface roughness, $n$ is the complex refractive index, and $\theta$ is the incidence angle from normal.

TODO argument on conservation of energy, and maintaining ratio of efficiencies?

By analogy from reflectivity (``fraction of incident light reflected'') to grating efficiency (``fraction of incident light reflected in order $n$''), we apply the same factor to determine the form of the roughness correction to apply to the grating efficiencies:
\begin{eqnarray}
e_n' &=& e_n \exp\left(  - \left(\frac{4\pi\sigma}{\lambda}\right)^2 \mathbf{Re}\left[ \sin \theta \sqrt{n^2 - \sin^2 \theta}  \right]    \right)
\end{eqnarray}
Since we don't have measurements of each grating's RMS roughness, we used $\sigma$ as a free parameter to attempt to fit the theoretical efficiencies to the measured efficiencies; the results are shown in Section \ref{gratingResults}.

Our AFM measurements show that re-coating the grating after ruling adds to the roughness, due to the granular nature of the evaporated coating.  Due to extended manufacturing experience, gold coatings are usually the smoothest to apply -- as was the case for our LEG.  The additional platinum and nickel coatings on the remaining gratings resulted in a much rougher surface.

Finally: TODO add note on ``geometric case'' when $\sigma >> \lambda$.  Microfacets... handle like BESSY: adds statistically random deviation to surface normal for each reflected ray, based on surface roughness. Howe exactly the angular probability distribution is computed, I don't know (whether this depends on energy).
...
\subsubsection{Irregularities in the groove position: ghost peaks}
